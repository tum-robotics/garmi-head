...
namespace head_control {
    ...
    public:
        ...
        HeadControlNode(const double k_p = 800.0, const double k_d = 50.0, const double pt1_filter = 0.001): k_p_(k_p), k_d_(k_d), pt1_filter_(pt1_filter), k_p_target_(k_p), k_d_target_(k_d), pt1_filter_target_(pt1_filter) {
        ...
        int init_controller() {
            // Control thread
            control_thread_ = std::thread(std::bind(&HeadControlNode::control_loop, this));
        ...
        int control_loop() {
            ...
            franka_joint_driver::Driver::CallbackFunctionTorque control_callback =
                    [&](const std::vector<Driver::State> &state,
                        ...
                        for (size_t joint_number = 0; joint_number < num_slaves_; joint_number++) {
                            ...
                            theta_d[joint_number] =
                                    theta_d[joint_number] * (1.0 - pt1_filter_) + pt1_filter_ * s_.get_theta_target(
                                            joint_number);
                            double tau_j = std::min(15.0, std::max(-15.0, (theta_d[joint_number] - theta) * k_p_ +
